...
export const teacherPlannerService = {
  ...
  // Bulk Operations
  async bulkDeletePlans(planIds: string[]): Promise<ApiResponse<{ deletedCount: number }>> {
    try {
      const response = await apiClient.delete('/teacher-planner/plans/bulk', {
        data: { planIds }
      });
      ...
      return {
        success: false,
        error: error.response?.data?.message || 'Failed to bulk delete plans'
      };
    }
  },

  async bulkUpdateStatus(planIds: string[], status: string): Promise<ApiResponse<{ updatedCount: number; newStatus: string }>> {
    try {
      const response = await apiClient.put('/teacher-planner/plans/bulk/status', {
        planIds,
        status
      });
      ...
      return {
        success: false,
        error: error.response?.data?.message || 'Failed to bulk update status'
      };
    }
  },

  async bulkDuplicatePlans(planIds: string[]): Promise<ApiResponse<{ duplicatedCount: number; plans: TeacherPlan[] }>> {
    try {
      const response = await apiClient.post('/teacher-planner/plans/bulk/duplicate', {
        planIds
      });
      ...
      return {
        success: false,
        error: error.response?.data?.message || 'Failed to bulk duplicate plans'
      };
    }
  },

  async bulkExportPlans(planIds: string[], format: 'json' | 'csv' = 'json'): Promise<ApiResponse<any>> {
    try {
      ...
      if (format === 'csv') {
        const response = await apiClient.get(`/teacher-planner/plans/bulk/export?${params}`, {
          responseType: 'blob'
        });
        ...
        link.href = url;
        link.setAttribute('download', `teacher-plans-export-${new Date().toISOString().split('T')[0]}.csv`);
        ...
        return {
          success: true,
          data: { message: 'CSV download started' }
        };
      } else {
        const response = await apiClient.get(`/teacher-planner/plans/bulk/export?${params}`);
      ...
      return {
        success: false,
        error: error.response?.data?.message || 'Failed to bulk export plans'
      };
    }
  },
